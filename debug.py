...
if not RESULTS.get("dep_torch") or not RESULTS.get("dep_transformers"):
    ...
    try:
        ...
        sub("Instantiate SentimentAgent (akan trigger background model load)")
        t0 = time.time()
        sa = SentimentAgent()
        ok(f"SentimentAgent instantiated dalam {time.time()-t0:.2f}s")
        ...
        for text, expected in test_cases:
            ...
            print(f"    {match} [{label:8s} {score:.3f}] {text[:50]}...")
            if label != expected:
                all_pass = False
        ...
        sub("Test sentiment pada artikel nyata")
        from backend.services.news_service import NewsService
        ns2 = NewsService()
        articles = ns2.fetch_news(limit=3)
        if articles:
            for a in articles:
                result = sa._aggregate_article_sentiment(a)
                score = result["score"]
                label = result["label"]
                is_default = abs(score - 0.5) < 0.001
                flag = f"{RED}← MASIH 0.5 (model tidak jalan!){RESET}" if is_default else ""
                print(f"    [{label:8s} {score:.3f}] {a.title[:50]}... {flag}")
        else:
